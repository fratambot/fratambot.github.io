...
If the most important thing for you is the band playing followed by your friends coming while you don't care about weather, the perceptron which best model this kind of decision-making would have for example $w_1 = 6$, $w_2 = 4$ and $w_3 = 0$. Finally, let's say that this perceptron has a threshold of 5 (bias, $b = -5$).
...
Using Eq. 3 we can easily verify that an input 00 ($x_1 = x_2 = 0$) will produce and output $o = 1$. The same happens for an input 01 and 10 while for an input 11 the output will be 0. This perceptron therefore implements a NAND gate.
...
where all perceptrons have a weights $w_1 = w_2 = -2$ and bias $b=3$ with the exception of the bottom right perceptron which has an explicitly indicated bias of -4.
...
However in order to work these algorithm need to tunes the parameters of a different kind of neurons capable of accepting as inputs and producing as output any number between 0 and 1 and not just 0 or 1.
...
It seems that sigmoid neurons are a completely different beast with respect to perceptrons but they are not, actually. In order to understand the similarities and differences between the two it suffice to take a look at the shape of the sigmoid function:
...
The sigmoid neuron can be considered as a smoothed perceptron and it is actualy the smoothness of the sigmoid function to play a crucial role rather than it's detailed form.
...
In here, and in the most simple examples you will find, the sigmoid function is used very often and that is, mainly, because exponential have very lovely properties when differentiated and the algebra when calculating the pertial derivatives before (Eq.6) is much easier with $\sigma$.
